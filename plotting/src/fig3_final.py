...
plt.rcParams.update({
    ...
    'axes.labelweight': 'normal',  # 坐标轴标签字体权重
    'axes.titleweight': 'semibold',  # 图表标题字体权重
})
...
for idx, data_csv in enumerate(data_csv_list):
    ...
    # ax.spines['bottom'].set_linewidth(9)  # 加粗下边框
    ...
    # ax.set_title(f'Paper_c{idx + 2} All Fields', fontsize=35)
    ...
    # inset_ax.spines['bottom'].set_linewidth(9)  # 加粗下边框
    # inset_ax.spines['left'].set_linewidth(9)
...
plt.savefig('fig3_final.pdf', format='pdf', bbox_inches='tight', pad_inches=0.2)
